...
public class CategoryController {
    ...
    public ResponseEntity<?> addNewCategory(@Valid CategoryRequest request, BindingResult result) {
        if (result.hasErrors()) {
            return ResponseEntity.badRequest()
                    .body(Map.of("status", false, "message", result.getFieldError().getDefaultMessage()));
        }
        try {
    ...
    public ResponseEntity<?> displayAllCategory() {
        return ResponseEntity.ok(Map.of("status", true, "categories", categoryService.getAllCategories()));
    }

    @PostMapping("/edit_category_data")
    public ResponseEntity<?> editCategoryWithIcon(@Valid CategoryRequest request, BindingResult result) {
